...
class Usuario extends Conectar
{
    ...
    /* Obtener sólo usuarios con rol de alumno para combo */
    public function get_usuario_alumno_combo()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT * FROM tm_usuario WHERE est = 1 AND rol_id = 1 ORDER BY usu_nom";
    ...
    public function get_cursos_x_usuario($usu_id)
    {
        ...
        $sql = "SELECT 
            ...
            tm_curso.cur_fecha_vencimiento,
            tm_usuario.usu_id,
            tm_usuario.usu_nom,
            tm_usuario.usu_apep,
            tm_usuario.usu_apem,
    ...
    public function get_total_cursos()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT count(*) as total FROM tm_curso WHERE est = 1";
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }

    /*TODO: Total de Alumnos */
    public function get_total_alumnos()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT count(*) as total FROM tm_usuario WHERE rol_id = 1"; // rol_id = 1 para alumnos
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }

    public function total_instructores()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT count(*) as total FROM tm_instructor WHERE est = 1";
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }

    public function total_certificados()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT count(*) as total FROM td_curso_usuario WHERE est = 1";
        $sql = $conectar->prepare($sql);
        $sql->execute();
        return $resultado = $sql->fetchAll();
    }
    ...
    //Insertar usuario desde excel Actualizado 18/11/24
    public function verificar_dni2($usu_dni)
    {
        $conectar = parent::conexion();
        parent::set_names();

        // Verificar si el DNI ya está registrado
        $sql = "SELECT * FROM tm_usuario WHERE usu_dni = ? AND est = 1";
        $stmt = $conectar->prepare($sql);
        ...
        if ($resultado && is_array($resultado)) {
            // Si el DNI ya está registrado, retornar error
            return ['status' => 'error', 'message' => 'El DNI ' . $usu_dni . ' ya está registrado'];
        } else {
            // Si el DNI no está registrado, retornar éxito
            return ['status' => 'success', 'message' => 'DNI disponible'];
        }
    }
    ...
    //Insertar usuario Actualizado 18/11/24
    ...
    public function insert_usuario($usu_nom, $usu_apep, $usu_apem, $usu_correo, $usu_pass, $usu_sex, $usu_telf, $rol_id, $usu_dni)
    {
        ...
        $sql = "INSERT INTO tm_usuario (usu_id, usu_nom, usu_apep, usu_apem, usu_correo, usu_pass, usu_sex, usu_telf, rol_id, usu_dni, fech_crea, est) 
                    VALUES (NULL,?,?,?,?,?,?,?,?,?,now(),'1')";
        $stmt = $conectar->prepare($sql);
        ...
        $stmt->bindValue(7, $usu_telf);
        $stmt->bindValue(8, $rol_id);
        $stmt->bindValue(9, $usu_dni);
        $stmt->execute();

        // Retornar mensaje de éxito después de insertar
        return json_encode(['status' => 'success', 'message' => 'Usuario registrado correctamente']);
    }


    //Insertar usuario desde excel Actualizado 18/11/24
    public function verificar_dni($usu_dni)
    {
        $conectar = parent::conexion();
        parent::set_names();
        ...
        if ($resultado && is_array($resultado)) {
            // Si el DNI ya está registrado, retornar error
            return ['status' => 'error', 'message' => 'DNI ya registrado'];
        } else {
            // Si el DNI no está registrado, retornar éxito
            return ['status' => 'success', 'message' => 'DNI disponible'];
        }
    ...
    // Método para insertar un usuario desde el Excel
    public function insert_usuario_excel($usu_nom,  $usu_correo, $usu_pass, $usu_sex, $usu_telf, $rol_id, $usu_dni)
    {
        $conectar = parent::conexion();
        parent::set_names();

        // Preparar la sentencia SQL para insertar
        $sql = "INSERT INTO tm_usuario (usu_nom, usu_correo, usu_pass, usu_sex, usu_telf, rol_id, usu_dni, fech_crea, est) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 1)";
        $stmt = $conectar->prepare($sql);
    ...
    public function update_imagen_firma($usu_id, $usu_foto)
    {
        $conectar = parent::conexion();
        parent::set_names();
    ...
    public function upload_file()
    {
        if (isset($_FILES["usu_foto"])) {
            ...
            $extension = pathinfo($_FILES['usu_foto']['name'], PATHINFO_EXTENSION);

            // Obtener dimensiones de la imagen
            list($width, $height) = getimagesize($_FILES['usu_foto']['tmp_name']);

            // Validar tamaño
            if ($width > $max_width || $height > $max_height) {
                // Tamaño excede el límite
                return "La imagen excede el tamaño máximo permitido (225*225 px).";
            }

            // Generar nuevo nombre y destino para la imagen
            $new_name = rand() . '.' . $extension;
            $destination = '../public/usuario_fotos/' . $new_name;

            // Mover la imagen
            move_uploaded_file($_FILES['usu_foto']['tmp_name'], $destination);

            // Devolver la ruta completa de la imagen para almacenar en la base de datos
            return "../../public/usuario_fotos/" . $new_name;
        }
    }
}
