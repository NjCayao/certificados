...
switch ($_GET["op"]) {
    ...
    case "guardaryeditar":
        ...
        if (isset($_POST["tiene_vencimiento"]) && $_POST["tiene_vencimiento"] == "si" && !empty($_POST["cur_fecha_vencimiento"])) {
            $fecha_vencimiento = $_POST["cur_fecha_vencimiento"];
        }

        if (empty($_POST["cur_id"])) {
            $curso->insert_curso($_POST["cat_id"], $_POST["cur_nom"], $_POST["cur_descrip"], $_POST["cur_fechini"], $_POST["cur_fechfin"], $_POST["inst_id"], $fecha_vencimiento);
        } else {
            $curso->update_curso($_POST["cur_id"], $_POST["cat_id"], $_POST["cur_nom"], $_POST["cur_descrip"], $_POST["cur_fechini"], $_POST["cur_fechfin"], $_POST["inst_id"], $fecha_vencimiento);
        }
        break;
    ...
    case "mostrar":
        ...
        if (is_array($datos) == true and count($datos) <> 0) {
            foreach ($datos as $row) {
                ...
                $output["cur_fecha_vencimiento"] = $row["cur_fecha_vencimiento"];
            }
    ...
    case "listar":
        ...
        foreach ($datos as $row) {
            ...
            // Mostrar fecha de vencimiento o "Sin vencimiento"
            if ($row["cur_fecha_vencimiento"] != null && $row["cur_fecha_vencimiento"] != '') {
                $sub_array[] = '<span class="badge badge-warning">' . $row["cur_fecha_vencimiento"] . '</span>';
            } else {
                $sub_array[] = '<span class="badge badge-success">Sin vencimiento</span>';
            }
