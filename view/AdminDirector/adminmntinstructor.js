...
function editar(inst_id){
    $.post("../../controller/instructor.php?op=mostrar",{inst_id : inst_id}, function (data) {
        ...
        // var urlFirma = '../../public/firma_img/' + data.inst_firma;
        // $('#imgFirma').attr('src', urlFirma);       
        // $('#divFirma').show();       
        
    });
    $('#lbltitulo').html('Editar Registro del Instructor');
...
function imagen(inst_id){
    $('#curx_idx').val(inst_id);
...
function guardaryeditarimg(e){
    ...
    $.ajax({
        url: "../../controller/instructor.php?op=update_imagen_firma",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
